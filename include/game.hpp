...
public:
	...
	Q_INVOKABLE void removePrompt(Prompt *prompt);
	void addReply(Prompt *prompt, const QString &text, QString target = nullptr);
	...
	Q_INVOKABLE void removeCharacter(const QString &key);
	...
	explicit Game()
		: m_scenario(new Scenario),
		  m_profile(new Profile(setting("Main/sEditModeProfile").toString(), m_scenario)),
